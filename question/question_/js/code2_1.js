...
//  $.get("./tripdatalist.txt",function(data){
  $.get("./tripdatalist2.txt",function(data){
      var temp = data.split("\n");
  ...
  // JSONファイル読み込み開始
  for (i=0; i<1; i++){
    //var num = Math.floor(Math.random()*3668);
    //num = 7462;
	num=document.getElementById("realid").value;
	patIDlist.push(num);
...
document.getElementById("setPatternP3Aselect").addEventListener("click",function(){
 ...
 document.getElementById("setPatternL8").addEventListener("click",function(){
  // JSONファイル読み込み開始
...
for (i=0; i<1; i++){
	...
	patIDlist.push(num);
    $('select#selectPattern').append($(document.createElement("option")).addClass("plist").text("ID:"+num));
    $.getJSON("./pat2/level8/"+num+".json",function(data){
...
document.getElementById("newResult").addEventListener("click",function(){
  // JSONファイル読み込み開始
  for (i=0; i<1; i++){
    var num = Math.floor(Math.random()*281);
    //num = 7462;
	patIDlist.push(num);
    $('select#selectPattern').append($(document.createElement("option")).addClass("plist").text("ID:"+num));
    $.getJSON("./pat3/pat"+num+".json",function(data){
  ...
  document.getElementById("setTripdata").addEventListener("click",function(){
    for (i=0; i<5; i++){
        ...
        $.getJSON("./trip3/"+tripdatalist[num],function(data){
  ...
  document.getElementById("tripToPattern").addEventListener("click",function(){
	...
	for(i=0; i<patIDlist.length; i++){
		...
		var data_num=0;	
			...
			if (ptr>=0){
				//console.log(tripdatalist[ptr]);
		
                          $.getJSON("./trip3/"+tripdatalist[ptr],function(data){
                          var getPath = createPath(data);
                          getPath.setMap(map);
                          triplinelist.push(getPath);
                          });
